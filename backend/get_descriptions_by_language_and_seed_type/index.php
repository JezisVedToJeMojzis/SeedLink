...
if (isset($_GET['language'], $_GET['seed_type'])) { //check if we received the needed values
    $pdo = new PDODatabaseManager($serverName, $userName, $userPassword, $databaseName); //connect to db
    $result = $pdo->getRecordingsByLanguageAndSeedType($_GET['language'], $_GET['seed_type']); //send them to PDO manager to work with db and save result of query into variable
    if ($result != null) {
