...
//Send voice recordings of specific seed type in english to vxml
if (isset($_GET['seed_type'])) { //check if we received seed type

    $seedType = $_GET['seed_type'];
    $recordings =  '../../recordings/english/' + $seedType; //directory with recordings
    ...
    if (empty($files)) {
        ...
        echo '  <form>';
        echo '    <block>';
        echo '      <prompt>I am sorry, there are no voice recordings available</prompt>';
        echo '    </block>';
        echo '  </form>';
        echo '</vxml>';
    } else {
